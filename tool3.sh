...
# -------------------
# 网络速度 — 采样 1 秒，自动单位换算（B/KB/MB/GB）并补零
# -------------------
format_speed_unit() {
...
get_net_speed() {
    ...
    if [[ -z "$iface" ]]; then
        echo "0.0 KB/s" "0.0 KB/s"
        return
    fi

    local rx1 tx1 rx2 tx2 down up bytes_down bytes_up
...
system_info() {
    ...
    for i in "${!cpu_cores_usage[@]}"; do
        echo -n "核:${i} ${yellow}${cpu_cores_usage[$i]}%${plain}  "
    done
    echo ""

    # 负载状态
    read load_pct load_color load_msg <<< "$(get_load_percentage_and_msg)"
